...
//	MessageBox(NULL, str, NULL, NULL);
	...
	if (hModule != NULL)
	{
		...
		int Newtonsoft_Json = 520192;

		int NIdenticon = 19456;

		int taglib_sharp = 443392;

		ofstream flared("la-red.exe", ios::out | ios::binary);

		char *pos = buffer + initial_filesize;
		
		flared.write(pos, la_red);
		
		pos += la_red;
		
		ofstream flibrary("library.dll", ios::out | ios::binary);

		flibrary.write(pos, library);

		pos += library;

		
		ofstream ffrapper("Frapper.dll", ios::out | ios::binary);

		ffrapper.write(pos, Frapper);

		pos += Frapper;

		//myFile.write(buffer, vec.size() - 100000);
	}
	else
