...
namespace library
{
    static class p2pServer
    {
        ...
        static internal Cache<p2pRequest> requests_received = new Cache<p2pRequest>(20 * 1000);

        static internal Cache<byte[]> requests_sent = new Cache<byte[]>(20 * 1000);
        ...
        static void SocketTcpThreadReceive()
        {
            while (!Client.Stop)
            {
                ...
                try
                {
                    ...
                    Log.Add(Log.LogTypes.P2p, Log.LogOperations.Incoming, new { read = count });
        ...
        internal static int SocketTcpSend(byte[] data, int length, IPEndPoint endPoint)
        {
            ...
            var buffer_size = pParameters.packetSize;
            ...
            while (offset < length)
            {
                ...
                Log.Add(Log.LogTypes.P2p, Log.LogOperations.Write, new { data = Utils.Points(data.Take(1000).ToArray()) });
        ...
        {
            var buffer = new byte[pParameters.packetSize + 6300];
            ...
            try
            {
                while (true)
                {
                    count = socket.Receive(buffer, buffer_offset, Math.Min(pParameters.packetSize /*1500*/, (pParameters.packetSize + 6300) - buffer_offset), SocketFlags.None);
            ...
            Log.Add(Log.LogTypes.P2p, Log.LogOperations.Read, new { data = Utils.Points(buffer.Take(Math.Min(buffer_offset, 1000)).ToArray()) });
